...
async def _audit(db: AsyncSession, admin_id: str, action: str, target_type: str, target_id: str | None, metadata: dict):
    await db.execute(
        text(
            """
...
def _login_response(response: Response, token: str, csrf: str, expires_at):
    settings = get_settings()
    _cookie(response, settings.admin_cookie_name, token, settings.admin_session_ttl_hours * 3600, httponly=True)
    _cookie(response, settings.admin_csrf_cookie_name, csrf, settings.admin_session_ttl_hours * 3600, httponly=False)
    return {"ok": True, "expires_at": expires_at}


@router.post("/auth/bootstrap")
async def bootstrap(body: BootstrapIn, request: Request, response: Response, db: AsyncSession = Depends(get_db)):
    settings = get_settings()
    if not settings.admin_bootstrap_token:
        raise HTTPException(503, "Admin bootstrap is disabled")
    if not __import__("hmac").compare_digest(body.bootstrap_token, settings.admin_bootstrap_token):
    ...
    admin_id = (
        await db.execute(
            text(
                "insert into admin_users(email,password_hash) values(:email,:hash) returning id"
            ),
            {"email": email, "hash": password_hash},
        )
    ).scalar_one()
    await _audit(db, str(admin_id), "admin.bootstrap", "admin_user", str(admin_id), {"email": email})
    token, csrf, expires = await create_admin_session(db, admin_user_id=str(admin_id), request=request)
    await db.commit()
...
async def login(body: LoginIn, request: Request, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    if not verify_admin_password(body.password, row["password_hash"]):
        ...
        if attempts >= 5:
            await db.execute(
                text(
                    "update admin_users set failed_login_attempts=0, locked_until=now() + interval '15 minutes' where id=:id"
                ),
                {"id": row["id"]},
            )
        else:
    ...
    await db.execute(
        text(
            "update admin_users set failed_login_attempts=0, locked_until=null, last_login_at=now() where id=:id"
        ),
        {"id": row["id"]},
    )
    token, csrf, expires = await create_admin_session(db, admin_user_id=str(row["id"]), request=request)
...
async def logout(response: Response, admin=Depends(require_admin_write), db: AsyncSession = Depends(get_db)):
...
async def put_econet(body: EconetConfigIn, admin=Depends(require_admin_write), db: AsyncSession = Depends(get_db)):
    if body.auth_scheme not in {"bearer", "api-key", "basic", "custom", "none"}:
        raise HTTPException(422, "Unsupported auth scheme")
    payload = body.model_dump()
    try:
        result = await save_econet_config(db, admin_id=str(admin["admin_user_id"]), payload=payload)
    except ValueError as exc:
        await db.rollback()
        raise HTTPException(422, str(exc)) from exc
    await _audit(db, str(admin["admin_user_id"]), "integration.save", "integration", result["id"], {
        "provider": "econet-ai",
...
async def test_econet(body: TestIn, admin=Depends(require_admin_write), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        await _audit(db, str(admin["admin_user_id"]), "integration.test", "integration", config.id, {
            "provider": "econet-ai",
            "kind": result["kind"],
        })
        await db.commit()
...
async def activate_econet(admin=Depends(require_admin_write), db: AsyncSession = Depends(get_db)):
    ...
    await db.execute(
        text("update integration_configs set enabled=false, updated_at=now(), updated_by=:admin_id where provider='econet-ai' and environment='production'"),
        {"admin_id": admin["admin_user_id"]},
    )
    await db.execute(
        text("update integration_configs set enabled=true, updated_at=now(), updated_by=:admin_id where id=:id"),
        {"id": config.id, "admin_id": admin["admin_user_id"]},
    )
    await _audit(db, str(admin["admin_user_id"]), "integration.activate", "integration", config.id, {"provider": "econet-ai"})
    await db.commit()
...
async def deactivate_econet(admin=Depends(require_admin_write), db: AsyncSession = Depends(get_db)):
    ...
    await db.execute(
        text(
            "update integration_configs set enabled=false, updated_at=now(), updated_by=:admin_id where id=:id"
        ),
        {"id": config.id, "admin_id": admin["admin_user_id"]},
    )
    await _audit(db, str(admin["admin_user_id"]), "integration.deactivate", "integration", config.id, {"provider": "econet-ai"})
    await db.commit()
    return {"ok": True, "active_provider": "demo"}

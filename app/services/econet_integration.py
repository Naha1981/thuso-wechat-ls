...
def _validate_public_endpoint(base_url: str, allow_private_network: bool = False) -> None:
...
def build_econet_config(row: dict, secrets: dict[str, Any]) -> EconetIntegration:
    return EconetIntegration(
        ...
        if row.get("health_endpoint_path")
        else None,
...
async def load_active_econet_config(db: AsyncSession) -> EconetIntegration | None:
    row = (
        await db.execute(
            text(
                """
                select id, environment, enabled, base_url, chat_endpoint_path,
                       health_endpoint_path, auth_scheme, auth_header_name,
                       model, timeout_seconds, request_template, response_mapping,
                       secret_ciphertext, last_test_status, last_test_at
    ...
    if row["secret_ciphertext"]:
        secrets = json.loads(SecretCipher(get_settings().secrets_encryption_key).decrypt(row["secret_ciphertext"]))
    return build_econet_config(dict(row), secrets)


async def get_saved_econet_config(
    db: AsyncSession,
) -> tuple[EconetIntegration | None, dict[str, Any]]:
    row = (
        await db.execute(
            text(
                """
...
async def save_econet_config(
    ...
    *,
    ...
    _validate_public_endpoint(payload["base_url"], bool(payload.get("allow_private_network")))
    ...
    if payload["auth_scheme"] not in {"bearer", "api-key"}:
        if payload["auth_scheme"] == "none":
            secrets_map.pop("api_key", None)
        if payload["auth_scheme"] == "basic":
            pass

    cipher = SecretCipher(settings.secrets_encryption_key)
    secret_ciphertext = cipher.encrypt(json.dumps(secrets_map, separators=(",", ":"))) if secrets_map else None
    ...
    if current:
        await db.execute(
            ...
            ),
            {
                ...
                if payload.get("health_endpoint_path")
                else None,
                ...
                text(
                    ...
                    values
                      ('econet-ai', 'production', false, :base_url, :chat_endpoint_path,
                       ...
                       cast(:response_mapping as jsonb), :secret_ciphertext,
                       :admin_id, :admin_id)
                    returning id
                    """
                ),
...
async def _request(
    ...
    *,
    ...
    _validate_public_endpoint(config.base_url, False)
    headers = {
        "Accept": "application/json",
        "X-NahaOS-Trace-Id": trace_id or "",
...
async def test_econet_config(
    config: EconetIntegration,
    *,
    smoke_chat: bool = False,
) -> dict[str, Any]:
    _validate_public_endpoint(config.base_url)
    ...
    return {"ok": True, "kind": "chat", "status_code": response.status_code, "content_preview": str(content)[:120]}


def provider_from_config(config: EconetIntegration):
